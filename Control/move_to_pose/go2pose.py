...
def move_to_pose(x_start, y_start, theta_start, x_goal, y_goal, theta_goal):
    ...
    while rho > 0.01:
        ...
        print("linear speed: " + str(v) + ', angular speed: ' + str(w / np.pi * 180))

        if show_animation:  # pragma: no cover
            plt.cla()
            plt.arrow(x_start, y_start, 0.1 * np.cos(theta_start),
                      0.1 * np.sin(theta_start), color='r', width=0.01)
            plt.arrow(x_goal, y_goal, 0.1 * np.cos(theta_goal),
                      0.1 * np.sin(theta_goal), color='g', width=0.01)
            plot_vehicle(x, y, theta, x_traj, y_traj)


def plot_vehicle(x, y, theta, x_traj, y_traj):  # pragma: no cover
    # Corners of triangular vehicle when pointing to the right (0 radians)
    p1_i = np.array([0.1, 0, 1]).T
    p2_i = np.array([-0.1, 0.05, 1]).T
    p3_i = np.array([-0.1, -0.05, 1]).T
...
def main():
    ...
    for i in range(5):
        # x_start = 1.5 - 3 * random()
        # y_start = 1.5 - 3 * random()
